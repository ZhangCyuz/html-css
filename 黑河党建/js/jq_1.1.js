	...
	addList:function(obj,cla,x){
		...
		for(var j=0;j<num;j++){
			...
			for(var i=0;i<n;i++){
				...
				s.push('<li>'+(cla[j]=='num'?'':(cla[j]=='txt'?this.$$('li',obj)[i].innerHTML.replace(/\<img.*?\>/i,this.$$('img',obj)[i].alt):(cla[j]=='thumb'?'<img src='+(this.$$('img',obj)[i].getAttribute("thumb")||this.$$('img',obj)[i].src)+' />':'')))+'<span>'+(i+1)+'</span></li>')
			};
		s.push('</ul>');
...
myFocus.setting({
	...
	id:'myFocus',//焦点图ID
	chip:10,//图片切片数量，能被焦点图的高整除才有效，默认为8片
